...
public class GongYingShangFragment extends BaseFragment {
    ...
    private RecyclerView recyclerviewGongYingShang;
    ...
    protected View initView() {
        ...
        recyclerviewGongYingShang = view.findViewById(R.id.recyclerview_gongyinshang);
    ...
    private void useRecyclerViewToShow() {
        //设置RecyclerView的适配器
        adapter = new ImageAndTestRecyclerViewAdapter(mContext, datas,0);
        recyclerviewGongYingShang.setAdapter(adapter);

        //LayoutManager
        //new GridLayoutManager 参数 1、上下文 2、列数 3、方向 4、是否倒序
        recyclerviewGongYingShang.setLayoutManager(new LinearLayoutManager(mContext, LinearLayoutManager.VERTICAL, false));
        //倒序后设置选显示倒序第一行
        //recyclerviewTime.scrollToPosition(datas.size()-1);

        //添加默认分割线：高度为2px，颜色为灰色
        recyclerviewGongYingShang.addItemDecoration(new RecycleViewDivider(mContext, LinearLayoutManager.VERTICAL));
    }
}
